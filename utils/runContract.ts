 ...
 * @returns {Promise<String>} spins up the contract via the Metamask API
 */
export const runContract = async (
  funcCallback?: (strValue: string) => void
) => {
  ...
  const strGeneratedResponse: string = await helloWorld.hello();

  if (!!funcCallback) {
    funcCallback(strGeneratedResponse);
  }

  return strGeneratedResponse;
};
